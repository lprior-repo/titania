...
pub fn check_crate_names(root: &Path, members: &[String], report: &mut LaneReport) {
    for member in members {
        check_crate_name(root, member, report);
    }
}
...
fn rust_files(root: &Path) -> Vec<PathBuf> {
    fs::read_dir(root)
        .map_or_else(|_| Vec::new(), |entries| {
            entries
                .flatten()
                .flat_map(|entry| rust_file_entry(entry.path()).into_iter())
                .collect()
        })
}
...
pub fn discover_members(root: &Path) -> Vec<String> {
    fs::read_to_string(root.join("Cargo.toml"))
        .map_or_else(|_| Vec::new(), |manifest| {
            quoted_array_values(&manifest, "members").into_iter().collect()
        })
}
