...
pub(crate) fn run_local_lane(target: &TargetProject, lane: LocalLane) -> LaneExit {
...
pub(crate) fn run_clippy_vb_compile(target: &TargetProject) -> LaneExit {
    cargo_status(
        target,
...
pub(crate) fn run_test(target: &TargetProject, group: &str) -> LaneExit {
    let args = vec!["test", "-p", "vb_compile", "--lib", group, "--", "--nocapture"];
    cargo_status(target, &args)
}

pub(crate) fn run_kani(target: &TargetProject, harness: &str) -> LaneExit {
    let args = vec!["kani", "--package", "vb_compile", "--harness", harness, "--quiet"];
    cargo_status(target, &args)
}

pub(crate) fn run_kani_default_unwind(target: &TargetProject, harness: &str) -> LaneExit {
    let args = vec![
        "kani",
        "--package",
        "vb_runtime",
