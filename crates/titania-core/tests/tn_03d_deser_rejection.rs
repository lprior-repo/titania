...
fn lane_deserialize_rejects_unknown_variant() {
    ...
    assert!(result.is_err(), "should produce a serde error");
}
...
fn lane_outcome_clean_rejects_nonzero_exit_on_deserialize() {
    // A clean outcome must not deserialize with non-zero process evidence.
    ...
    let outcome = serde_json::from_value::<titania_core::LaneOutcome>(outcome_json);
    assert!(outcome.is_err(), "clean outcome with non-zero exit must reject");
}
