...
fn lane_from_str_empty_string_rejected() {
    ...
    assert!(
        matches!(result, Err(titania_core::LaneError::UnknownLane(..))),
...
fn report_reject_rejects_both_empty() {
    ...
    let result = titania_core::Report::reject(findings, failures, per_lane);
    assert!(
        matches!(result, Err(titania_core::ReportError::EmptyReject)),
...
fn location_span_line_start_min_is_1() {
    ...
    assert!(
        matches!(result, Err(titania_core::LocationError::LineStartBeforeOne)),
...
fn repair_hint_all_7_variants_constructible() {
    ...
    assert!(matches!(
        titania_core::RepairHint::use_iterator_pipeline("use iterators".to_string()),
        titania_core::RepairHint::UseIteratorPipeline { .. }
    ));
    assert!(matches!(
        titania_core::RepairHint::flatten_nesting("reduce nesting".to_string()),
        titania_core::RepairHint::FlattenNesting { .. }
    ));
    assert!(matches!(
        titania_core::RepairHint::use_checked_arithmetic("checked_add".to_string()),
        titania_core::RepairHint::UseCheckedArithmetic { .. }
    ));
    assert!(matches!(
        titania_core::RepairHint::remove_allow_attribute("allow(dead_code)".to_string()),
        titania_core::RepairHint::RemoveAllowAttribute { .. }
    ));
    assert!(matches!(
        titania_core::RepairHint::replace_dependency(
            "old-dep = \"0.1\"".to_string(),
            "new-dep = \"0.2\"".to_string(),
        ),
        titania_core::RepairHint::ReplaceDependency { .. }
    ));
    assert!(matches!(
        titania_core::RepairHint::requires_human_review("manual fix needed".to_string()),
        titania_core::RepairHint::RequiresHumanReview { .. }
    ));
}
...
fn repair_hint_patch_with_zero_width_range_rejected() {
    ...
    assert!(
        matches!(result, Err(titania_core::RepairHintError::EmptyRange)),
...
fn make_invalid_exit_evidence() -> Result<titania_core::LaneEvidence, titania_core::OutcomeError> {
    let cmd = titania_core::CommandEvidence::new(
        "cargo".to_string(),
        vec!["cargo".to_string(), "fmt".to_string()].into_boxed_slice(),
    )
...
fn lane_outcome_clean_rejects_nonzero_exit() {
    let outcome = make_invalid_exit_evidence();
    assert!(
        matches!(outcome, Err(titania_core::OutcomeError::NonZeroExit)),
...
fn lane_outcome_clean_rejects_timed_out() {
    ...
    assert!(matches!(evidence, Err(titania_core::OutcomeError::NonZeroExit)));
}
...
fn command_evidence_rejects_empty_argv() {
    ...
    assert!(
        matches!(result, Err(titania_core::OutcomeError::EmptyArgv)),
...
fn command_evidence_rejects_argv0_mismatch() {
    ...
    assert!(
        matches!(result, Err(titania_core::OutcomeError::Argv0Mismatch { .. })),
...
fn command_evidence_rejects_single_empty_string_argv() {
    ...
    assert!(matches!(
        result,
        Err(titania_core::OutcomeError::EmptyArgv)
            | Err(titania_core::OutcomeError::Argv0Mismatch { .. })
    ));
}
...
fn process_termination_signaled_rejects_signal_0() {
    ...
    assert!(
        matches!(result, Err(titania_core::FailureError::InvalidSignal(_))),
...
fn quality_receipt_rejects_wrong_schema_version() {
    ...
    let bad = serde_json::json!({
        ...
        "cargo_lock_digest": Digest::from_bytes(b"lock"),
        "policy_digest": Digest::from_bytes(b"policy"),
        "toolchain_digest": Digest::from_bytes(b"toolchain"),
        "lanes": [],
    });
    let result = serde_json::from_value::<titania_core::QualityReceipt>(bad);
    assert!(result.is_err(), "wrong schema_version should be rejected");
}
...
fn quality_receipt_empty_lanes_allowed() {
    ...
    assert!(matches!(result, titania_core::QualityReceipt { .. }));
}
